...
class PNPD_NN_step(nn.Module):
    ...
    def forward(
        ...
        net_input = torch.cat(
            (u, observed), dim=1
        )  # Stack u and observed along the channel dimension
        # net_input = u

        # Update stepsizes
        self.params.alpha = self.alpha_Net(net_input) + 0.5
        ...
        lam_regularization = lam_regularization[
            :, None, None
        ]  # Add dimensions to match lam_prox input
        self.funs.prox_h_star = lambda x, lam_prox: self.prox_h_star(
            lam_regularization, x, lam_prox
        )
