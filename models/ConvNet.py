...
class ConvNet(nn.Module):
    def __init__(
        ...
        self.conv_layers = [
            nn.Conv2d(num_filters[i], num_filters[i + 1], kernel_size=kernel_sizes[i])
        ...
        fc_hidden_layers = (
            [final_shape[0] * final_shape[1] * num_filters[-1]]
            + fc_hidden_layers
            + [num_outputs]
        )
        self.fc_layers = [
            nn.Linear(fc_hidden_layers[i], fc_hidden_layers[i + 1])
