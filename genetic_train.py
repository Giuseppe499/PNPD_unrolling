...
def next_generation(population, fitness_scores, population_size, model_factory):
  ...
  children = [generate_child(parent1, parent2, model_factory) for parent1, parent2 in parents_pairs]
  children = [mutate(child) for child in children]
  return children

def genetic_train(population_size, model_factory, training_loader, fitness_fn, unrolling_steps, n_best_models=5, device="cpu", grad_steps=1):
  ...
  for generation in range(100):  # Example: 100 generations
    for i, model in enumerate(population):
      fitness_scores[i] = fitness(model, training_loader, device=device, fitness_fn=fitness_fn, unrolling_steps=unrolling_steps)
      while torch.isnan(fitness_scores[i]):
        print(f"Model {i} produced NaN loss, regenerating.")
        population[i] = model_factory().to(device)
        fitness_scores[i] = fitness(population[i], training_loader, device=device, fitness_fn=fitness_fn, unrolling_steps=unrolling_steps)
      ...
      if torch.isnan(fitness_scores[i]):
        fitness_scores[i] = -100 # Assign a large negative value to NaN fitness scores
    ...
    population = next_generation(population, fitness_scores, population_size, model_factory)
...
if __name__ == "__main__":
  ...
  train_loader = DataLoader(
    training_set, batch_size=len(training_set), shuffle=False, num_workers=num_workers
  )
  ...
  model_factory = lambda: PNPD_NN_step(
    alpha_Net=Net(),
    beta_Net=Net(),
    nu_Net=Net(),
    lam_Net=Net(),
